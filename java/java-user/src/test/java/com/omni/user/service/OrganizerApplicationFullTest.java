...
class OrganizerApplicationFullTest {
    OrganizerApplicationMapper am; UserMapper um; PlatformTransactionManager tm; TransactionStatus ts;
    ...
    @BeforeEach void setup() {
        am = mock(OrganizerApplicationMapper.class); um = mock(UserMapper.class);
        ...
        svc = new OrganizerApplicationService(am, um, tm);
    }
    ...
    class Review {
        ...
        void oa011() { when(um.selectById(any())).thenReturn(u(2002L,"admin")); when(am.selectById(any())).thenReturn(a(10L,2004L,0)); when(am.update(isNull(),any())).thenReturn(1); assertEquals(1, svc.approve(2002L,10L,"ok").getStatus()); }
        @Test @DisplayName("OA-012: upgrade role")
        void oa012() { User applicant = u(2004L,"user"); when(um.selectById(any())).thenReturn(u(2002L,"admin"), applicant); OrganizerApplication ap = a(10L,2004L,0); ap.setOrganizerName("NewOrg"); when(am.selectById(any())).thenReturn(ap); when(am.update(isNull(),any())).thenReturn(1); svc.approve(2002L,10L,"ok"); assertEquals("organizer",applicant.getRole()); assertEquals(1,applicant.getOrganizerStatus()); assertEquals("NewOrg",applicant.getOrganizerName()); }
        @Test @DisplayName("OA-013: reject→REJECTED")
        void oa013() { when(um.selectById(any())).thenReturn(u(2002L,"admin")); when(am.selectById(any())).thenReturn(a(10L,2004L,0)); when(am.update(isNull(),any())).thenReturn(1); assertEquals(2, svc.reject(2002L,10L,"bad").getStatus()); }
        @Test @DisplayName("OA-014: reject no note→400")
        void oa014() { when(um.selectById(2002L)).thenReturn(u(2002L,"admin")); when(am.selectById(10L)).thenReturn(a(10L,2004L,0)); assertThrows(BusinessException.class,()->svc.reject(2002L,10L,"")); }
        @Test @DisplayName("OA-015: organizer→403")
        void oa015() { when(um.selectById(2003L)).thenReturn(u(2003L,"organizer")); assertThrows(BusinessException.class,()->svc.approve(2003L,10L,"try")); }
        @Test @DisplayName("OA-016: re-review→rejected")
        void oa016() { when(um.selectById(2002L)).thenReturn(u(2002L,"admin")); when(am.selectById(10L)).thenReturn(a(10L,2004L,1)); assertThrows(BusinessException.class,()->svc.approve(2002L,10L,"again")); }
    }
    ...
    class Permission {
        ...
        void oa019() { when(um.selectById(2002L)).thenReturn(u(2002L,"admin")); when(am.selectById(999999L)).thenReturn(null); assertThrows(BusinessException.class,()->svc.approve(2002L,999999L,"ok")); }
    }
}
