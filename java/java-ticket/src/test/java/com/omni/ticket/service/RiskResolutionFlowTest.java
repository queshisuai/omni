...
class RiskResolutionFlowTest {
    ...
    class PermissionErrors {
        ...
        void rr018() { when(uas.requirePlatformPermission(2002L,"risk.review")).thenReturn(null); when(rm.selectById(999999L)).thenReturn(null); ActivityRiskResolutionReviewRequest r = new ActivityRiskResolutionReviewRequest(); r.setUserId(2002L); r.setAction("approve"); assertThrows(BusinessException.class,()->svc.reviewResolution(999999L,r)); }
    }
    ...
    class Query {
        ...
        void rr014() { when(uas.requirePlatformPermission(2002L,"risk.view")).thenReturn(null); when(am.selectList(any())).thenReturn(List.of(activity(5L,"risk_suspended",2003L))); when(rm.selectList(any())).thenReturn(Collections.emptyList()); List<ActivityRiskCaseResponse> r = svc.listRiskCases(2002L); assertTrue(r.size()>=0); }
    }
