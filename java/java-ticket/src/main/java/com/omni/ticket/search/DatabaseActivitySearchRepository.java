...
@Repository
public class DatabaseActivitySearchRepository {
    ...
    public Page<ActivityVO> filter(Page<ActivityVO> source, ActivitySearchRequest request) {
        ...
        List<ActivityVO> sourceRecords = source == null || source.getRecords() == null
                ? List.of()
                : source.getRecords();
        int safePage = actualRequest.getPage() == null || actualRequest.getPage() <= 0 ? 1 : actualRequest.getPage();
        int safeSize = actualRequest.getSize() == null || actualRequest.getSize() <= 0 ? 10 : actualRequest.getSize();
        List<ActivityVO> filtered = sourceRecords.stream()
                .filter(vo -> matchesKeyword(vo, actualRequest.getKeyword()))
                .filter(vo -> matchesCity(vo, actualRequest.getCity()))
                .filter(vo -> matchesDate(vo, actualRequest.getDateFrom(), actualRequest.getDateTo()))
                .filter(vo -> matchesPrice(vo, actualRequest.getMinPrice(), actualRequest.getMaxPrice()))
                .filter(vo -> matchesSaleStatus(vo, actualRequest.getSaleStatus()))
                .filter(vo -> !Boolean.TRUE.equals(actualRequest.getSeatMapOnly()) || "published".equals(vo.getSeatMapVisibility()))
                .filter(vo -> actualRequest.getRealNameRequired() == null
                        || Boolean.valueOf(actualRequest.getRealNameRequired()).equals(Boolean.TRUE.equals(vo.getRealNameRequired())))
                .collect(Collectors.toList());
        filtered.sort(searchComparator(actualRequest.getSort()));
