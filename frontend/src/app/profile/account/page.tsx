...
export default function ProfileAccountPage() {
  ...
  const handleSendPasswordCode = async () => {
    ...
    try {
      ...
      setPasswordMessage('验证码已发送，请按短信提示输入。')
    } catch (err: unknown) {
              ...
              <section className="rounded-3xl border border-[#ececec] bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:p-8">
                <div className="mb-6 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#666]">提交后即时生效，不会强制登出；验证码由后端真实校验。</p>
                  </div>
                </div>
                ...
                <form className="grid gap-4 sm:grid-cols-2" onSubmit={handlePasswordSubmit}>
                  ...
                  <div className="sm:col-span-2 flex flex-col gap-3 border-t border-[#f0f0f0] pt-6 mt-2 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-[13px] text-[#666] max-w-[280px]">{passwordMessage || '密码长度建议不少于 6 位；请按短信提示输入验证码。'}</p>
                    <button
                      type="submit"
