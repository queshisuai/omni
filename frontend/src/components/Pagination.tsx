...
export function Pagination({ page, total, pageSize = DEFAULT_PAGE_SIZE, loading = false, onChange }: PaginationProps) {
  ...
  const goToPage = (nextPage: number) => {
    if (loading) return
    const normalized = Math.min(Math.max(1, nextPage), pages)
    if (normalized !== current) onChange(normalized)
  }

  return (
    <div className="mt-4 flex flex-col gap-3 text-[13px] text-[#666] sm:flex-row sm:items-center sm:justify-between">
      <div>
